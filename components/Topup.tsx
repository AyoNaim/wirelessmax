...
const API_BASE_URL = "https://wirelessmaxnetwork.com.ng/app/api/fund-wallet/index.php";
...
export default function FundAccountPage() {
  ...
  const handleRefresh = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        "https://wirelessmaxnetwork.com.ng/app/api/user/app-refresh/index.php",
        {
          method: "POST",
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const bankResponse = await fetch(
        "https://wirelessmaxnetwork.com.ng/app/api/user/sitesettings/index.php"
      );
      const bankResult = await bankResponse.json();
      if (bankResult.status === "success" && bankResult.data) {
        setAdminBankDetails({
          bankname: bankResult.data.bankname,
          accountname: bankResult.data.accountname,
          accountno: bankResult.data.accountno,
        });
      }
    } catch (error) {
  ...
  const handleGenerateAccount = async () => {
    ...
    try {
      const response = await fetch(
        `https://wirelessmaxnetwork.com.ng/app/debug.php?token=${encodeURIComponent(
          token
        )}`,
        {
          method: "POST",
  ...
  const submitManualVerification = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://wirelessmaxnetwork.com.ng/app/api/fund-wallet/manual/index.php",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            ...
            >
              ...
              <div className="flex justify-between items-center mb-4">
                <div>
                  ...
                  <h4 className="font-bold">{adminBankDetails.bankname}</h4>
                </div>
                <div className="text-right">
                  <p className="text-xs text-zinc-500">Account Name</p>
                  <h4 className="font-bold">{adminBankDetails.accountname}</h4>
                </div>
              </div>
              <div
              ...
              >
                <div>
                  ...
                  <p className="text-lg font-black tracking-wider">
                    {adminBankDetails.accountno}
                  </p>
                </div>
                <Button
                  onClick={() => copyToClipboard(adminBankDetails.accountno, "admin_acc")}
                  variant="ghost"
                  size="icon"
